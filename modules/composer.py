...
# Background music ki awaaz (voiceover = 1.0 ke muqable mein).
# Pehle 0.05 tha jo lagbhag sunai hi nahi deta tha. 0.12 halki lekin
# saaf sunai deti hai aur voice ko dabati nahi. Zyada chahiye to 0.15,
# kam chahiye to 0.09 kar dena.
BG_MUSIC_VOLUME = 0.12

# Scenes ke beech bohot chhota gap (seconds) taake awaaz natural lage.
SCENE_GAP = 0.05
...
class ShortsComposer:
    ...
    def _fit_vertical(clip):
        """Clip ko 1080x1920 (9:16) mein fill karta hai - zaroorat ho to center crop."""
    ...
    def _prepare_scene_video(path, duration):
        """Ek scene ka clip: sahi length, random starting point, 9:16."""
    ...
    def _add_background_music(self, voice_audio, total_duration, bg_music_path):
        ...
        if bg_music_path and os.path.exists(bg_music_path):
            print(f"🎵 Background music mil gaya: {bg_music_path} (volume {BG_MUSIC_VOLUME})")
    ...
    # ------------------------------------------------------------------
    # NEW: har scene ka apna clip + apni voice (multi-clip video)
    # ------------------------------------------------------------------
    def create_multi_scene_short(
        self,
        clip_paths,
        voiceover_paths,
        ...
        print("🎬 Multi-scene video composition start ho rahi hai...")
        ...
        try:
            ...
            print(f"⏱️ Total video length: {total_duration:.1f}s")
    ...
    # ------------------------------------------------------------------
    # OLD single-clip method (kept as is, backward compatible)
    # ------------------------------------------------------------------
