    ...
    **kwargs
    ...
    try:
        ...
        if len(clips) == 1:
            ...
            # NOTE: method="compose" wraps everything in a CompositeVideoClip.
            # In moviepy 1.0.3 that path has a known bug where, if any tiny
            # gap/rounding mismatch happens between clip durations, it tries
            # to call `self.bg.get_frame(t)` while `self.bg` is None, which
            # crashes with "'NoneType' object has no attribute 'get_frame'"
            # right at render time. Every clip is already resized to the
            # same 1080x1920 above, so we don't need the compositing layer
            # at all - "chain" just plays clips back-to-back and avoids the
            # bug entirely.
            try:
