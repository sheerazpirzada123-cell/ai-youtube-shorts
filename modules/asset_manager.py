...
def prepare_background_audio(
    ...
    """
    Pixabay se free background music aur sound effects download karta hai
    (koi API key nahi chahiye - ye direct CDN files hain). Composer.py
    already assets/audio/bg_music.mp3 aur assets/sfx/*.mp3 ko automatically
    pick kar leta hai agar wo maujood hon, is liye is function ko call karne
    ke baad music/SFX khud-ba-khud final video mein lag jate hain.

    Agar download fail ho jaye (no internet, URL dead, etc.) to sirf skip
    kar deta hai - pipeline kabhi is wajah se crash nahi hogi, video bina
    music/SFX ke ban jayegi.
    """

    os.makedirs(os.path.dirname(bg_music_path) or ".", exist_ok=True)
    ...
    if not (
        ...
        try:
            print("Downloading background music from Pixabay...")
            download_file(BGM_URL, bg_music_path, asset_type="audio")
        except Exception as e:
            print(f"Background music download skipped: {e}")

    sfx_sources = {
        "whoosh.mp3": WHOOSH_SFX_URL,
        "pop.mp3": POP_SFX_URL,
    }
    ...
    for filename, url in sfx_sources.items():
        ...
        if os.path.exists(target) and os.path.getsize(target) > 1000:
            continue
        try:
            print(f"Downloading sound effect from Pixabay: {filename}")
            download_file(url, target, asset_type="audio")
        except Exception as e:
            print(f"Sound effect '{filename}' download skipped: {e}")
