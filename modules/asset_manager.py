...
def fetch_pixabay_clip(
    ...
    query_candidates += random.sample(

        PEXELS_SEARCH_TERMS,

        k=min(2, len(PEXELS_SEARCH_TERMS))
    )
...
def prepare_background_audio(
    ...
    """
    Pixabay se free sound effects download karta hai (koi API key nahi
    chahiye - ye direct CDN files hain). Composer.py already
    assets/sfx/*.mp3 ko automatically pick kar leta hai agar wo maujood
    hon, is liye is function ko call karne ke baad SFX khud-ba-khud final
    video mein lag jate hain.

    NOTE: Background music jaan-boojh kar yahan se hata di gayi hai - user
    ne bataya ke wo sunayi nahi de raha tha, is liye ab sirf sound effects
    add hote hain.
