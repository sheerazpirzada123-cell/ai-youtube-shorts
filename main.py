...
async def main():
    ...
    # 3. ASSETS: Get Stock Video (+ local actor photos if you've added any)
    asset_manager = AssetManager()
    # topic comes back as "Actor Name - short angle"; the part before " - "
    # is the actor's name, used to look up assets/actor_photos/<slug>/.
    actor_name = topic.split(" - ")[0].strip() if " - " in topic else topic
    try:
        assets_map = asset_manager.get_videos(script, topic=topic, actor_name=actor_name)
    except Exception as e:
